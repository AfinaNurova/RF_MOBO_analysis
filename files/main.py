...
N_BATCH = 70 if not SMOKE_TEST else 10
...
def main():
    ...
    print(f"Starting optimization for {gp_model_type} and {dataset} data, {acquisition}, with seed {seed}")

    train_x, train_obj_true = generate_initial_data(problem, n=2 * (problem.dim + 1))
    base_train_data = (train_x.clone(), train_obj_true.clone())

    # "qParEGO", "qQEHVI", "qQNEHVI", 
    # for acquisition in ["Sobol"]:
    log_rows = []
    metadata = {
        "dataset": dataset,
    ...
    while iteration < N_BATCH and check_hv_early_stopping(hvs):
        ...
        log_entry = {
            ...
            "objective_mean": torch.mean(new_obj, dim=0, keepdim= True).cpu().numpy().tolist(),
            "objective_std": torch.std(new_obj, dim=0, keepdim= True).cpu().numpy().tolist(),
            "candidate_norms": torch.norm(new_x, dim=0).cpu().numpy().tolist(),
            "hypervolume": volume,
        ...
        if acquisition != "Sobol":
            ...
            log_entry["posterior_std"] = stddev.detach().cpu().numpy().tolist()
        else:
            log_entry["posterior_std"] = None

        hv_improvement = volume - hvs[-2] if len(hvs) > 1 else 0
        log_entry["hypervolume_improvement"] = hv_improvement
